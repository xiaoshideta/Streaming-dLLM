...
class LLaDAEvalHarness(LM):
    ...
    def generate_until(self, requests):
            ...
            if self.use_cache:
                if self.dual_cache:
                    ...
                    generated_answer, nfe = generate_with_prefix_cache(self.model, input_ids, steps=self.steps, gen_length=self.gen_length, block_length=self.block_length, 
                                        temperature=0, remasking=self.remasking, mask_id=self.mask_id, eos_id=self.eos_id, threshold=self.threshold, factor=self.factor, window_size=self.window_size, confidence_alpha=self.confidence_alpha)
            else:
        ...
        if self.show_speed:
            ...
            stats_data = {
                "total_generated_tokens": num_tokens.item(),
                "total_generation_time_seconds": end_time - start_time,
                "tokens_per_second": (num_tokens.item()) / (end_time - start_time),
                "NFE": num_nfe,
                # "diffusion_steps": self.steps,
            }
            
            current_time = datetime.now().strftime("%Y%m%d_%H%M%S")
            filename = f"generation_stats_{current_time}.json"
